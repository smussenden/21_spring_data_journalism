...
covid_county <- read_csv("data/covid_county_current_08_31_2020.csv") %>%
...
covid_state <- read_csv("data/covid_state_current_08_31_2020.csv") %>%
  filter(!state %in% c("Northern Mariana Islands","Puerto Rico","Virgin Islands", "Guam"))  %>%
  dplyr::select(fips_code = fips, state, date, cases, deaths)
  
covid_us <- read_csv("data/covid_us_current_08_31_2020.csv") %>%
  mutate(location="US") %>%
